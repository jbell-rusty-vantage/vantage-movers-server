 ...
 * Core, candidate set and the dry-run prediction: `ops/lib/reensure-outreach.ts`.
 *
 *   node --env-file=.env --import tsx ops/reensure-outreach.ts --allow-production [--limit=N] [--batch=25]     # dry run (read only)
 *   node --env-file=.env --import tsx ops/reensure-outreach.ts --apply --allow-production                        # operator, after steps 1–5
 *   node --env-file=.env --import tsx ops/reensure-outreach.ts --apply --allow-production --resume               # continue a stopped apply
 *   node --env-file=.env --import tsx ops/reensure-outreach.ts --apply --allow-production --release-holds        # operator decision: hand held paid jobs back
 *
