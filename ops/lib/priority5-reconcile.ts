 ...
 * Used by `ops/reconcile-priority5.ts` and its replica proof `test-si-priority5.replica.test.ts`.
 *
...
export function priority5Report(manifest: P5Manifest, env: string, flags: Record<string, boolean>): string {
  ...
  return [
    ...
    `- Script: \`ops/reconcile-priority5.ts\` (${P5_RECONCILE_VERSION}); flags ${Object.entries(flags).map(([k, v]) => `${k}=${v}`).join(", ")}`,
