 ...
 * Entry point: `ops/reensure-outreach.ts`. Replica proof: `ops/test-si-reensure.replica.test.ts`.
 *
...
export function reensureReport(cp: ReensureCheckpoint, env: string, flags: Record<string, boolean>, files: { checkpoint: string; jsonl: string; summary: string }): string {
  ...
  return [
    ...
    `- Script: \`ops/reensure-outreach.ts\` (${cp.version}); \`--no-progress-plan\` ${cp.no_progress_plan ? "on" : "OFF"}; flags ${Object.entries(flags).map(([k, v]) => `${k}=${v}`).join(", ")}`,
