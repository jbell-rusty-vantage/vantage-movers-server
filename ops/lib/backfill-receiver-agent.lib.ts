 ...
 * Used by `ops/backfill-receiver-agent.ts` and its replica proof `ops/test-si-receiver-agent.ts`.
 *
...
export function receiverBackfillReport(manifest: ReceiverBackfillManifest, env: string, flags: Record<string, boolean>): string {
  ...
  return [
    ...
    `- Script: \`ops/backfill-receiver-agent.ts\` (${RECEIVER_BACKFILL_VERSION}); flags ${Object.entries(flags).map(([k, v]) => `${k}=${v}`).join(", ")}`,
