...
import { activateGranotLifecycle } from "../services/granotLifecycle/operations";
import {
  projectGranotJob,
...
router.post("/api/v1/admin/granot-lifecycle/activation", async (req, res) => {
  try {
    ...
    const data = await activateGranotLifecycle(command, actor);
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const data = await projectGranotJob(raw);
...
router.get("/api/v1/admin/granot-lifecycle/operations/health", async (req, res) => {
  try {
    await connectMongo();
    requireRegistryReadActor(req, auth(req));
    const data = await projectGranotLifecycleHealth();
    return res.json({ ok: true, data });
  } catch (error) {
    return sendError(res, error, requestId(req));
  }
