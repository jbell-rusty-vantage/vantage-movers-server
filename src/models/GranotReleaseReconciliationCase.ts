...
export type GranotReleaseCaseState = "open" | "resolved";
export type GranotReleaseCaseNoActionReasonCode =
  (typeof GRANOT_RECONCILIATION_NO_ACTION_REASON_CODES)[number];

export type GranotReleaseCaseEvidence = {
...
export type GranotReleaseReconciliationCaseDocument = {
  ...
  resolution?: {
    outcome: (typeof GRANOT_RELEASE_RECONCILIATION_OUTCOMES)[number];
    command_execution_id: mongoose.Types.ObjectId;
    actor: DurableActor;
    reason_code?: GranotReleaseCaseNoActionReasonCode;
    reason_text?: string;
    resolved_at: Date;
...
const releaseEvidenceSchema = new Schema(
  {
    ...
    action: { type: String, required: true, enum: ["release"] },
  },
  { _id: false },
);

const GranotReleaseReconciliationCaseSchema =
  new Schema<GranotReleaseReconciliationCaseDocument>(
    {
      normalized_job_no: { type: String, required: true, trim: true },
      job_no_snapshot: { type: String, required: true, trim: true },
      action_kind: { type: String, required: true, enum: ["release"], default: "release" },
      sequence_number: { type: Number, required: true, min: 1, validate: Number.isInteger },
      state: {
        type: String,
        required: true,
        enum: GRANOT_RECONCILIATION_CASE_STATES,
        default: "open",
      },
      case_revision: {
        type: Number,
      ...
      deterministic_booking_id: { type: Schema.Types.ObjectId, required: true },
      booking_revision_at_open: {
        type: Number,
        required: true,
        min: 0,
        validate: Number.isInteger,
      },
      evidence: { type: [releaseEvidenceSchema], required: true, default: [] },
      observed_context: {
        type: new Schema(
          {
            contact: {
      ...
      resolution: {
        type: new Schema(
          {
            outcome: { type: String, required: true, enum: GRANOT_RELEASE_RECONCILIATION_OUTCOMES },
...
for (const index of GRANOT_RELEASE_RECONCILIATION_CASE_INDEXES) {
  ...
  GranotReleaseReconciliationCaseSchema.index(index.key, options);
}

GranotReleaseReconciliationCaseSchema.post("init", function rememberImmutableState(doc) {
  doc.$locals.persisted_state = doc.state;
  doc.$locals.persisted_evidence_ids = doc.evidence.map((row) => String(row.observation_id));
});

GranotReleaseReconciliationCaseSchema.pre("validate", function rejectForbiddenMutation() {
  if (this.isNew) return;
  if (this.$locals.persisted_state === "resolved") {
    throw new Error("A resolved Release case is immutable");
  }
  ...
  if (before.some((id) => !current.has(id))) {
    throw new Error("Existing Release case evidence IDs are immutable");
  }
});

GranotReleaseReconciliationCaseSchema.pre(
  ...
  function rejectUnsafeDirectCaseUpdate() {
    ...
    if (!Object.keys(update).some((key) => key.startsWith("$"))) {
      throw new Error("Release reconciliation cases cannot be replaced directly");
    }
    if (filter.state !== "open") {
      throw new Error("Release reconciliation case updates must guard on open state");
    }
    const set = (update.$set ?? {}) as Record<string, unknown>;
    if (set.state === "open") {
      throw new Error("A resolved Release case cannot return to open");
    }
    if ("evidence" in set || "$pull" in update || "$pop" in update) {
      throw new Error("Existing Release case evidence IDs are immutable");
    }
  },
);

export const GranotReleaseReconciliationCase: Model<GranotReleaseReconciliationCaseDocument> =
  (mongoose.models[GRANOT_RELEASE_RECONCILIATION_CASE_MODEL_NAME] as
    | Model<GranotReleaseReconciliationCaseDocument>
    | undefined) ??
  mongoose.model<GranotReleaseReconciliationCaseDocument>(
    GRANOT_RELEASE_RECONCILIATION_CASE_MODEL_NAME,
    GranotReleaseReconciliationCaseSchema,
  );

export function getGranotReleaseReconciliationCaseModel(): Model<GranotReleaseReconciliationCaseDocument> {
  const dbName = getMongoDatabaseName();
  if (mongoose.connection.name === dbName) return GranotReleaseReconciliationCase;
  const db = mongoose.connection.useDb(dbName, { useCache: true });
  return (
    (db.models[GRANOT_RELEASE_RECONCILIATION_CASE_MODEL_NAME] as
      | Model<GranotReleaseReconciliationCaseDocument>
      | undefined) ??
    db.model<GranotReleaseReconciliationCaseDocument>(
      GRANOT_RELEASE_RECONCILIATION_CASE_MODEL_NAME,
      GranotReleaseReconciliationCaseSchema,
    )
  );
