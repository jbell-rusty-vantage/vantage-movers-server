...
/**
 * One transaction, several audit rows. Use when a single Owner command
 * must create multiple registry records or none.
 */
export async function withMultiEntityRegistryMutation<T>(
  input: {
    actor: RegistryActorContext;
    invalidateKeys?: string[];
    mutate: (
      session: ClientSession,
      writeAudit: (audit: RegistryAuditInput) => Promise<void>,
    ) => Promise<T>;
  },
  deps: RegistryAuditDeps = {},
): Promise<T> {
  ...
  try {
    result = await runTransaction(async (session) => {
      return input.mutate(session, async (audit) => {
        await writeAudit(session, {
          ...audit,
          actor: input.actor,
        });
