// B22 regression baseline (S4-TIMELINE): the Subject Story readers exactly as they were before the timeline
// work (server 0781162, `story/sources.ts`), byte for byte below this header. Test-only: `b22.test.ts` runs these
// readers and the current ones over the same in-memory data and requires identical output. Never import it from
// production code; update it only together with an intended change to the model's story page.
