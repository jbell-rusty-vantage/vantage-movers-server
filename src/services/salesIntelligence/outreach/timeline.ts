...
export const outreachTimelineSource: TimelineSource = async ({ number_id, cursor, limit }) => {
  ...
  const keys = [`number:${number_id}`, ...edges.map(e => `lead:${e.lead_ref.model}:${e.lead_ref.id}`)];
  const rows = await getSalesIntelligenceAuditEventModel().aggregate([
    { $match: { subject_key: { $in: keys }, "invalidation.kind": { $in: ["outreach", "followup", "review", "restriction"] }, event_kind: { $nin: ["clock_boundary", "outreach_interaction", "outreach_number_linked"] } } },
