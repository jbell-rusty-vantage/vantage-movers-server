...
async function maybeReconcileBooking(input: {
  ...
  if (result.kind === "booking_discrepancy_required") {
    return persistProcessorDiscrepancy({
      discrepancy_kind: result.reason_code.startsWith("release_") ? "release" : "booking",
...
async function defaultLoadLeadProjection(
  ...
  if (target.model === "FormLead") {
    ...
    return {
      ...
      receiver_agent_source: absent(row.receiver_agent_source),
      receiver_agent_set_at: row.receiver_agent_set_at ? new Date(row.receiver_agent_set_at) : undefined,
