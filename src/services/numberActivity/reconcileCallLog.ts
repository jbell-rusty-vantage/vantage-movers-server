  ...
  const applyOne = async (record: CallLogRecordInput): Promise<ApplyOutcome> => {
    ...
    try {
      const applied = await deps.apply(
        ...
        // INTEGRATION: pass `settleHorizonMinutes: deps.config.settleHorizonMinutes` once
        // `applyInteractionObservation` accepts it (CC-04 settles a record quiet past the horizon).
        { now: deps.now, directory: directory!, resolveRoute, request_id: runRequestId },
      );
