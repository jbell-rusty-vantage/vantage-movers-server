...
test("requireApiSecret accepts the global API secret on any v1 route", () => {
...
test("requireApiSecret accepts a source-scoped key for its configured route", () => {
...
test("requireApiSecret rejects a scoped key for a different source company", () => {
  delete process.env.VANTAGE_API_SECRET;
  process.env.VANTAGE_SCOPED_API_KEYS = JSON.stringify([
    {
      name: "best-relocation-forms",
...
test("requireApiSecret rejects unknown secrets", () => {
  process.env.VANTAGE_API_SECRET = "global-secret";
  process.env.VANTAGE_SCOPED_API_KEYS = JSON.stringify([
    {
      name: "best-relocation-forms",
