...
// S10 step 2 isolated replica proof (C25: `backfill-sole-match-attachments.ts`). Independent of .env and any inherited Atlas URI.
...
const database = `testvantagemovers_t3csole${randomBytes(5).toString("hex")}`;
const child = spawn(process.execPath, ["--import", "tsx", "--import", "./scripts/test-setup.ts", "--test", "scripts/dev_ops/test-si-sole-match-backfill.replica.test.ts"], {
  stdio: "inherit",
  env: {
    ...process.env,
...
console.log(`S10-2 sole-match isolated replica proof: ${database}; loopback replica csi01 on port 27189`);
child.on("exit", code => { process.exitCode = code ?? 1; });
child.on("error", () => { console.error("Unable to start isolated S10-2 replica proof"); process.exitCode = 1; });
