...
// S2-DESK isolated replica proof (B6, B7, flag on/off, index read). Independent of .env and any inherited Atlas URI.
const database = "testvantagemovers_s2desk";
const child = spawn(
  process.execPath,
  ["--import", "tsx", "--import", "./scripts/test-setup.ts", "--test", "scripts/dev_ops/test-si-desk.replica.test.ts"],
  {
    stdio: "inherit",
    env: {
      ...process.env,
      ...
      // The test passes the flag per publish; the environment default stays off as in production.
      SALES_INTELLIGENCE_ATTENTION_V2: "false",
...
console.log(`S2-DESK isolated replica proof: ${database}; loopback replica csi01 on port 27189`);
child.on("exit", (code) => {
  process.exitCode = code ?? 1;
});
child.on("error", () => {
  console.error("Unable to start isolated S2-DESK replica proof");
  process.exitCode = 1;
});
