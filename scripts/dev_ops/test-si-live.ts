...
// S5c-LIVE isolated replica proof (B2, B3, publish read count). Independent of .env and any inherited Atlas URI.
const database = "testvantagemovers_t3live";
const child = spawn(
  process.execPath,
  ["--import", "tsx", "--import", "./scripts/test-setup.ts", "--test", "scripts/dev_ops/test-si-live.replica.test.ts"],
  {
    stdio: "inherit",
    env: {
      ...process.env,
...
console.log(`S5c-LIVE isolated replica proof: ${database}; loopback replica csi01 on port 27189`);
child.on("exit", (code) => {
  process.exitCode = code ?? 1;
});
child.on("error", () => {
  console.error("Unable to start isolated S5c-LIVE replica proof");
  process.exitCode = 1;
});
