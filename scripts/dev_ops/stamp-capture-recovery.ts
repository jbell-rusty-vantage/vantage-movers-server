 ...
 *     node --env-file=.env --import tsx scripts/dev_ops/stamp-capture-recovery.ts \
 *       --manifest scripts/dev_ops/output/call-log-repair-2026-09-20.json --allow-production
 *   apply (from the deployed commit; the production-writer guard refuses otherwise):
 *     node --env-file=.env --import tsx scripts/dev_ops/stamp-capture-recovery.ts \
 *       --manifest scripts/dev_ops/output/call-log-repair-2026-09-20.json --apply --allow-production
 *
...
export function renderReport(r: StampReport): string {
  ...
  return [
    ...
    "",
    `Generated by \`scripts/dev_ops/stamp-capture-recovery.ts\` (${r.version}). Identifiers and counts only.`,
    "",
    "| Fact | Value |", "|---|---|",
...
async function main() {
  ...
  await runStampCaptureRecovery({ manifestPath: resolve(manifestPath), apply, outDir: resolve(option("--out") ?? "scripts/dev_ops/output"), reportPath });
}
