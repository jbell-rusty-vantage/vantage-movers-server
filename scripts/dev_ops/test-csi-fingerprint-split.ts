...
/** AC1 K3/K4 replica proof: `node --import tsx scripts/dev_ops/test-csi-fingerprint-split.ts` (disposable database). */
const database = `testvantagemovers_t4cfp${randomBytes(5).toString("hex")}`;
...
const child = spawn(process.execPath, ["--import", "tsx", "--import", "./scripts/test-setup.ts", "--test", "--test-reporter=tap", "scripts/dev_ops/test-csi-fingerprint-split.replica.test.ts"], {
...
console.log(`AC1 fingerprint split proof: ${database}; loopback replica csi01:27189; synthetic evidence only`);
