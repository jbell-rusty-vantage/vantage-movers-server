...
const database = `testvantagemovers_ma${randomBytes(6).toString("hex")}`;
...
const child = spawn(process.execPath, ["--import", "tsx", "--import", "./scripts/test-setup.ts", "--test", "scripts/dev_ops/test-csi-move-assessment.replica.test.ts"], {
  stdio: "inherit",
  env: {
...
console.log(`MA-02 Move assessment disposable proof: ${database}; loopback replica csi01:27189; mocked model, synthetic evidence only`);
child.on("exit", code => { process.exitCode = code ?? 1; });
child.on("error", () => { console.error("Unable to start the Move assessment replica proof"); process.exitCode = 1; });
