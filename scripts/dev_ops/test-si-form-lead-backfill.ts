...
// V-T3 M5a isolated replica proof (S10 step 1: `backfill-form-lead-contact-numbers.ts` holds its paid work). Independent of .env and any inherited Atlas URI.
...
const database = `testvantagemovers_t3bform${randomBytes(5).toString("hex")}`;
const child = spawn(process.execPath, ["--import", "tsx", "--import", "./scripts/test-setup.ts", "--test", "scripts/dev_ops/test-si-form-lead-backfill.replica.test.ts"], {
  stdio: "inherit",
  env: {
    ...process.env,
    ...
    SALES_INTELLIGENCE_PRIORITY5_CLOSURE: "true", SALES_INTELLIGENCE_FORM_LEAD_NUMBERS: "true", AI_GATEWAY_API_KEY: "",
...
console.log(`V-T3 M5a form-lead backfill isolated replica proof: ${database}; loopback replica csi01 on port 27189`);
child.on("exit", code => { process.exitCode = code ?? 1; });
child.on("error", () => { console.error("Unable to start the M5a replica proof"); process.exitCode = 1; });
