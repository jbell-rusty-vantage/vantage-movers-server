...
// S1-FACTS isolated replica proof (B2, B3, publish read count). Independent of .env and any inherited Atlas URI.
const database = "testvantagemovers_s1facts";
const child = spawn(
  process.execPath,
  ["--import", "tsx", "--import", "./scripts/test-setup.ts", "--test", "scripts/dev_ops/test-si-facts.replica.test.ts"],
  {
    stdio: "inherit",
    env: {
      ...process.env,
...
console.log(`S1-FACTS isolated replica proof: ${database}; loopback replica csi01 on port 27189`);
child.on("exit", (code) => {
  process.exitCode = code ?? 1;
});
child.on("error", () => {
  console.error("Unable to start isolated S1-FACTS replica proof");
  process.exitCode = 1;
});
