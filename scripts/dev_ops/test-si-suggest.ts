...
// S1-SUGGEST isolated replica proof (card line 6 case 2: `outreach.suggested_next_step` on the detail and the desk row, Apply parity with apply_suggestion).
// Loopback csi01 only; no model call, every network call is refused, synthetic evidence only.
const database = `testvantagemovers_s1suggest${randomBytes(6).toString("hex")}`;
...
const child = spawn(process.execPath, ["--import", "tsx", "--import", "./scripts/test-setup.ts", "--test", "scripts/dev_ops/test-si-suggest.replica.test.ts"], {
  stdio: "inherit",
  env: {
...
console.log(`S1-SUGGEST disposable proof: ${database}; loopback replica csi01:27189; no model, synthetic evidence only`);
child.on("exit", code => { process.exitCode = code ?? 1; });
child.on("error", () => { console.error("Unable to start the S1-SUGGEST replica proof"); process.exitCode = 1; });
