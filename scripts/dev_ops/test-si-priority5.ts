...
// S6-P5 isolated replica proof (C1, C2, C22, 5 ↔ 7 ↔ 8, flag-off identity, the S10-4 reconcile). Independent of .env and any inherited Atlas URI.
const database = `testvantagemovers_t3dp5${randomBytes(5).toString("hex")}`;
const child = spawn(process.execPath, ["--import", "tsx", "--import", "./scripts/test-setup.ts", "--test", "scripts/dev_ops/test-si-priority5.replica.test.ts"], {
  stdio: "inherit",
  env: {
    ...process.env,
    ...
    SALES_INTELLIGENCE_PRIORITY5_CLOSURE: "true",
...
console.log(`S6-P5 isolated replica proof: ${database}; loopback replica csi01 on port 27189`);
child.on("exit", code => { process.exitCode = code ?? 1; });
child.on("error", () => { console.error("Unable to start isolated S6-P5 replica proof"); process.exitCode = 1; });
