 ...
 *
 *   node --env-file=.env --import tsx scripts/dev_ops/report-si-state.ts --allow-production [--before=<S10-6 summary>] [--since=<ISO>]
 *
...
export function renderSiStateReport(r: SiStateReport): string {
  ...
  return [
    ...
    `- Database \`${r.database}\`; as_of ${r.as_of}; generated ${r.generated_at}. Read only (\`scripts/dev_ops/report-si-state.ts\`).`,
...
async function main() {
  ...
  const reportDir = arg("report-dir") ?? (existsSync(WORKSPACE_EVIDENCE) ? WORKSPACE_EVIDENCE : "scripts/dev_ops/output");
