...
const database = `testvantagemovers_csistructured${randomBytes(6).toString("hex")}`;
...
const child = spawn(process.execPath,["--import","tsx","--import","./scripts/test-setup.ts","--test","scripts/dev_ops/test-csi-structured.replica.test.ts"],{
  stdio:"inherit",env:{...env,CSI_REPLICA_TEST:"true",TEST_MODE:"true",TEST_MONGO_DATABASE_NAME:database,
    ...
    VANTAGE_ADMIN_PROXY_SIGNING_SECRET:"synthetic-structured-owner-signature",SALES_INTELLIGENCE_ANALYSIS_V3:"true",VANTAGE_API_SECRET:"synthetic-global",VANTAGE_SCOPED_API_KEYS:JSON.stringify([{name:"synthetic-intelligence",secret:"synthetic-scoped",routes:[
...
console.log(`Structured CSI disposable proof: ${database}; loopback replica csi01:27189; synthetic evidence only`);
