...
// MA-03 isolated replica proof (assessment-only backfill runner: inventory, shadow, caps, resume, promotion).
// Loopback csi01 only; the model is mocked and every network call is refused.
const database = `testvantagemovers_mabf${randomBytes(6).toString("hex")}`;
...
const child = spawn(process.execPath, ["--import", "tsx", "--import", "./scripts/test-setup.ts", "--test", "scripts/dev_ops/test-csi-move-assessment-backfill.replica.test.ts"], {
  stdio: "inherit",
  env: {
...
console.log(`MA-03 Move assessment backfill disposable proof: ${database}; loopback replica csi01:27189; mocked model, synthetic evidence only`);
child.on("exit", code => { process.exitCode = code ?? 1; });
child.on("error", () => { console.error("Unable to start the Move assessment backfill replica proof"); process.exitCode = 1; });
