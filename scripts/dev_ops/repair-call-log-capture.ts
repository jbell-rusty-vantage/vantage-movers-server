 ...
 *   node --env-file=.env --import tsx scripts/dev_ops/repair-call-log-capture.ts \
 *     --from 2026-09-21T00:00:00Z --to 2026-09-22T00:00:00Z --manifest scripts/dev_ops/output/<name>.json --allow-production
 * Write:
 *   node --env-file=.env --import tsx scripts/dev_ops/repair-call-log-capture.ts \
...
async function main() {
  ...
  const manifestPath = resolve(option("--manifest") ?? `scripts/dev_ops/output/call-log-repair-${confirm ? "apply" : "dry-run"}.json`);
