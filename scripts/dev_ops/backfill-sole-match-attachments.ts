 ...
 * - resumable: apply saves `scripts/dev_ops/output/sole-match-attachment-backfill.checkpoint.json`
 ...
 *
 *   node --env-file=.env --import tsx scripts/dev_ops/backfill-sole-match-attachments.ts --allow-production                  # dry run
 *   node --env-file=.env --import tsx scripts/dev_ops/backfill-sole-match-attachments.ts --job=5564884 --allow-production     # one job's Numbers
 *   node --env-file=.env --import tsx scripts/dev_ops/backfill-sole-match-attachments.ts --apply --allow-production [--resume] [--limit=N]
 *   node --env-file=.env --import tsx scripts/dev_ops/backfill-sole-match-attachments.ts --apply --allow-production --release-holds
 *
...
const OUT_DIR = arg("out") ?? "scripts/dev_ops/output";
...
function renderReport(input: {
  ...
  return [
    ...
    `- Script: \`scripts/dev_ops/backfill-sole-match-attachments.ts\` (${SOLE_MATCH_POLICY_VERSION}); \`${input.argv}\`${input.job ? `; job ${input.job}` : ""}`,
