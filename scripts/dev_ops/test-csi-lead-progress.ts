...
const database = `testvantagemovers_lp01${randomBytes(6).toString("hex")}`;
const child = spawn(process.execPath, ["--import", "tsx", "--import", "./scripts/test-setup.ts", "--test", "scripts/dev_ops/test-csi-lead-progress.replica.test.ts"], {
  stdio: "inherit",
  env: {
    ...process.env,
...
console.log(`LP-01 isolated replica proof: ${database}; loopback replica csi01 on port 27189`);
child.on("exit", code => { process.exitCode = code ?? 1; });
child.on("error", () => { console.error("Unable to start isolated LP-01 replica proof"); process.exitCode = 1; });
