...
// S7-CLOSED isolated replica proof (C14). Independent of .env and any inherited Atlas URI.
const database = "testvantagemovers_t3cclosed";
const child = spawn(
  process.execPath,
  ["--import", "tsx", "--import", "./scripts/test-setup.ts", "--test", "scripts/dev_ops/test-si-closed-history.replica.test.ts"],
  {
    stdio: "inherit",
    env: {
      ...process.env,
...
console.log(`S7-CLOSED isolated replica proof: ${database}; loopback replica csi01 on port 27189`);
child.on("exit", (code) => {
  process.exitCode = code ?? 1;
});
child.on("error", () => {
  console.error("Unable to start isolated S7-CLOSED replica proof");
  process.exitCode = 1;
});
