...
/** AC7-BACKFILL estimate replica proof: `node --import tsx scripts/dev_ops/test-csi-case-file-backfill-estimate.ts` (disposable database). */
const database = `testvantagemovers_t4cbf${randomBytes(5).toString("hex")}`;
...
const child = spawn(process.execPath, ["--import", "tsx", "--import", "./scripts/test-setup.ts", "--test", "--test-reporter=tap", "scripts/dev_ops/test-csi-case-file-backfill-estimate.replica.test.ts"], {
...
console.log(`AC7-BACKFILL estimate proof: ${database}; loopback replica csi01:27189; synthetic evidence only`);
