...
async function getSourceLeadStats(models: HistoricalModels) {
  const groupPipeline: PipelineStage[] = [
    {
      $group: {
        ...
        total_leads: { $sum: 1 },
        booked_leads: {
          $sum: { $cond: [{ $ne: [{ $ifNull: ["$booked", null] }, null] }, 1, 0] },
        },
        cancelled_leads: {
          $sum: { $cond: [{ $ne: [{ $ifNull: ["$cancelled", null] }, null] }, 1, 0] },
        },
  ...
  for (const row of callStats) {
    ...
    existing.sheet_booked_leads = Number(existing.sheet_booked_leads) + row.booked_leads;
    existing.sheet_cancelled_leads = Number(existing.sheet_cancelled_leads) + row.cancelled_leads;
  ...
  return Array.from(bySource.values()).sort((a, b) => {
    return Number(b.total_deposit_amount ?? 0) - Number(a.total_deposit_amount ?? 0);
  });
}
  ...
  const pipeline: PipelineStage[] = [
    {
      $set: {
        ...
        delivery: {
          $cond: [{ $or: [{ $eq: ["$delivery_state", null] }, { $eq: ["$delivery_state", ""] }] }, "unknown", "$delivery_state"],
        },
      },
    },
    {
    ...
    models.BookedLead.aggregate([
      ...addBookedLeadSourceLookups(),
      {
        $group: {
          ...
          bookings: { $sum: 1 },
          linked_bookings: {
            $sum: { $cond: [{ $ne: [{ $ifNull: ["$lead_ref", null] }, null] }, 1, 0] },
          },
        },
      },
      {
        $project: {
          ...
          link_rate: {
            $cond: [{ $eq: ["$bookings", 0] }, 0, { $divide: ["$linked_bookings", "$bookings"] }],
          },
        },
      },
