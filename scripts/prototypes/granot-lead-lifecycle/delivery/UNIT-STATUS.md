...
- Unit 16 is complete on `granot-lead-lifecycle` plus `granot_sync_extensions_and_services/main` (`0.2.8`). Re-verify `UNIT-16-COMPLETION.md` before depending on extension receipt apply. Unit 17 remains independently implementable after Unit 15 and must not reconstruct patches or desired state. If it overlaps the shared capture/envelope seam, prefer extending the Unit 16 channel-capture helper rather than a second insert path.
...
- Unit 18's contract is complete but implementation remains blocked until Unit 17 is implemented and a designated integration owner accepts combined webhook/extension/automation shadow parity evidence. Checked-in defaults remain shadow true with Lead writes/creation/cases false.
