...
| 24 | Confirm missing standard Booking owner workflow | 10–11, 22–23, Owner review | ready | complete | — |
...
- Unit 24 is now ready. Its Units 10–11/22–23 prerequisites and the Owner-review gate are satisfied. Begin only after repository re-verification of those completion reports, migrations/indexes, flags, and tests. It lands confirm-missing capability with the shared Booking command flag still false. Production merge must keep TEST_MODE, Mongo target, protection-bypass, API host, and every effect flag exact; Preview wiring mistakes must not be copied into production.
- Unit 25 remains implementation-blocked. It waits for verified Unit 24, owns existing-Booking update and standard Booking No Action, and completes S16 before any separately authorized narrow Booking-command rollout.
