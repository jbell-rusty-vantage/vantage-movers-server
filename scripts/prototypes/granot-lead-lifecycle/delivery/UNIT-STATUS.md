...
- Unit 22 is complete on `granot-lead-lifecycle`. It lands the exact evidence-rich Booking-case model and five indexes, live/gate-only processor integration, atomic Decision+case open/refresh, append/dedupe revision semantics, one-retry concurrent open/sequence convergence, deterministic Booking/employee-reconciliation/discrepancy routing, safe 24-hour candidate projection, masked audits, and current-cardinality gauges. The Booking-case flag and every command/email flag remain false. Unit 23 is now ready and owns read/API/Admin deployment before any separately approved Booking-case enablement; owner commands remain blocked for Units 24–25.
- Unit 23 is complete on `granot-lead-lifecycle` in both server and Admin. Its focused, full, typecheck/lint/build, Unit 22 replica prerequisite, Unit 23 zero-mutation replica proof, and aggregate index verify are green. The explicitly authorized Atlas `testvantagemovers` apply created 42 previously missing predecessor index definitions after a zero-collision report; no production apply or deployment occurred.
- Units 24–25 have complete implementation contracts but remain implementation-blocked. Unit 24's Units 10–11/22–23 prerequisites are now satisfied, but it still waits for explicit Owner review of the deployed read-only Booking workflow; it lands confirm-missing capability with the shared Booking command flag still false. Unit 25 waits for verified Unit 24, owns existing-Booking update and standard Booking No Action, and completes S16 before any separately authorized narrow Booking-command rollout.
