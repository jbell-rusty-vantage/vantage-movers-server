...
- **Specification and AC references:** Sections 14 and 34.3/34.5; AC-10/11/12 prerequisites.
...
deterministic/unknown ingestion-origin report, normalized Job backfill, honest `legacy_baseline` snapshots, duplicate/bad/collision inventory, indexes, and verify commands.
...
- no business values may be rewritten by provenance migration.
...
- **Approved unit-specific proof:** idempotent migration tests; dry-run changes no documents; apply on disposable fixtures changes only approved metadata; unknown evidence is never mislabeled original; output is PII-safe.
