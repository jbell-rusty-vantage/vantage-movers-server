...
- **Specification and AC references:** final-spec Sections 9–18, 26–28, 33–36, 39, and 41; this unit certifies existing AC behavior and introduces no new domain behavior.
...
replay **current Granot webhook payload shapes** through the completed authenticated webhook/capture/processor/application-logic path in an isolated local/staging test environment, produce a PII-safe certification report, and make the final go/no-go decision.

## 3. Repositories and fixed branches

- **Affected repositories:** primarily server, with admin/extension observation only if an asserted cross-channel result requires it.
...
Units 01–31 and 33 complete; Unit 32 complete only if included; ordinary full regression green; no unresolved migration/index/security failures.
...
- **Why separate:** current payload shapes introduce privacy, schema-drift, and environment risk. They certify the completed design; they are not ad hoc implementation inputs for earlier units.
- **Detailed protocol:** follow Section 8 of the approved 34-unit handoff in full. Raw current payloads must never be committed or included in hook, subagent, model, issue, or report output. Only sanitized structural derivatives may become durable fixtures.
- **Failure rule:** a mismatch creates a follow-up defect against the owning earlier unit/contract. Do not weaken normalization, identity, source policy, authority, or invariants merely to accept an unexpected payload.
- **Completion evidence:** produce a signed/dated PII-safe matrix of payload family → Receipt → Observation → Decision → permitted effect/no-effect, prove every expected assertion, prove no credential/contact leakage or external effects, and rerun the full regression after any approved fix.
...
- Must be made exact during contract refinement from the cited S-slice and final-spec flag posture.
...
- **Approved unit-specific proof:** a signed/dated PII-safe certification matrix covering payload family → Receipt → Observation → Decision → permitted effect/no-effect; all expected assertions; no leaked credentials/contact; no external effects; and full regression green after any approved fixes.
