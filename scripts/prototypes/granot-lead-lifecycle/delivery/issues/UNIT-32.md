...
- **Original slice:** S22.
- **Specification and AC references:** Section 32 and S22.
...
typed Notification Delivery purpose/case reference, new-sequence-only dedupe, templates, provider-safe failure behavior, and delivery metrics.
...
all case workflows accepted by Owner; explicit approval to include email in the release.
...
- **Optionality:** this unit is included only after explicit Owner approval for email in the release. Omitting it produces a 33-unit program without renumbering or weakening any dependency.
- **Delivery isolation:** dashboard cases remain the authoritative work items. Email is a deduplicated projection only and provider failure cannot alter or block a case.
...
- **Approved unit-specific proof:** one email per newly opened sequence; none on evidence refresh; provider failure cannot alter/block a case; sandbox/test recipient only until separately approved.
...
email flag off leaves dashboard/cases unaffected.
