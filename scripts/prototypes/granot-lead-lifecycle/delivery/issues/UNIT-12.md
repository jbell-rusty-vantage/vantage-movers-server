...
- **Specification and AC references:** Sections 14.2–14.4 and 15; field prerequisites for AC-03/07/10/11/12.
...
immutable Ingestion Origin, Form Job Number parity, normalized Job fields, Priority/Granot fields, immutable contact/move snapshots, provenance summaries, temporal winner, convergence state, indexes, creation-path snapshot capture, and strict public/admin write exclusions.
...
- Lead lifecycle writes and creation remain disabled.
...
- **Approved unit-specific proof:** model/service tests for origin derivation, immutable fields, WordPress snapshot transaction, Granot/RingCentral-specific creation permissions, `post_to_granot=false`, Call `quoted` parity, and no public mutation of internal metadata.
