...
- **Original slice:** S11.
- **Specification and AC references:** Sections 28.1 and 31; automation portions of AC-02/33.
...
`${run_id}:${action_id}` receipt identity, statement construction from locked plan, Owner initiator, processor invocation, lifecycle refs/outcome on run action receipt, accepted-for-processing lease yielding, exact replay/conflict behavior, and preserved preview/approval/checksum protections.

## 3. Repositories and fixed branches

- **Affected repositories:** server and existing admin automation display only as required.
...
- no legacy apply path may bypass receipt processing.
...
- **Approved unit-specific proof:** module/run-workflow/route tests for one lifecycle receipt per action, continuation using same operation ID, non-terminal checkpoint behavior, terminal replay, hash mismatch, immutable plan preservation, and equivalent shadow desired state to webhook evidence.
