...
- **Original slice:** S03.
- **Specification and AC references:** Sections 7 and 10; AC-05/06 and action aliases from AC-25/29.
...
one Observation per receipt; exact scalar, source, identity, contact, move, Priority, money, Agent, and Booking Action normalization; invalid/unsupported completion vocabulary.
...
- shadow-safe normalization only; no target matching or aggregate mutation.
...
- **Approved unit-specific proof:** pure and model tests across webhook encodings and extension/automation operation kinds; exact `Booked`, `Releas`, and `Release` handling; explicit rejection of `Released`; Priority edge cases including `0`, `05`, large allowed values, malformed/missing values, and safe raw preservation.
