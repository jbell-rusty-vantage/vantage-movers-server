...
- **Specification and AC references:** Sections 8.2–8.3, 29 as applicable, 34.2, 38/S04; AC-09/29/38.
...
exact-normalized source inventory/join, `GranotAutomationSource` reference adapter, locked initial classifications, unmatched/ambiguous report, audited enable/edit UX where needed, and PII-safe report/apply/verify commands.

## 3. Repositories and fixed branches

- **Affected repositories:** server; admin only for the minimum reviewed mutation/display surface needed.
...
- migration must leave `lead_created_policy=link_only`; creation is enabled only later in Unit 19.
...
- **Approved unit-specific proof:** migration tests demonstrate no guessed IDs, disabled/deferred ambiguous rows, exact Best Relocation Form/Call routing, Referral/Paid Overflow/Auto behavior, compatible automation reads, and visible compatibility errors.
