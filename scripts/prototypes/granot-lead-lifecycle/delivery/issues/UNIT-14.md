...
- **Specification and AC references:** Sections 8.4, 12–13, 15.4, 16 identity portions; AC-04/07/09/13/29.
...
resolve Registry policy before identity; Form and Call ladders; exact-identity source checks; Record Link lookup; Bad/Duplicate eligibility; source-scoped current+immutable contact matching; Agent assertion/match; deterministic Booking identity context.
...
- returns candidates/explanations only; no desired-state mutation.
...
- **Approved unit-specific proof:** pure/module tests for every ladder rung, ambiguity, conflicts, bad/duplicate rules, ObjectId compatibility, no global contact match, exact Source Scope enforcement, `user`/`rep` conflict, and Booking-missing-Lead delegation to existing reconciliation.
