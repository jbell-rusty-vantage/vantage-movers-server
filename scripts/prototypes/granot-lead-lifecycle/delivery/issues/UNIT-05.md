...
- **Specification and AC references:** Sections 8.1 and 8.4, plus Section 23 provenance expectations; AC-04/09/29/38.
...
`GranotCrmSource` semantic fields, strict route/disposition/policy validation, trusted Registry commands, transactionally paired audit, cache invalidation after commit, layered effect-gate evaluation, and runtime fail-closed resolution.
...
- all lifecycle rows remain disabled/deferred by default.
...
- **Approved unit-specific proof:** model/service tests for normalized labels, illegal/ambiguous routes, inactive scopes, legal creation policy combinations, audit atomicity, cache invalidation ordering, and gate snapshots.
