...
const database = `testvantagemovers_csi06${randomBytes(6).toString("hex")}`;
const child = spawn(
  process.execPath,
  [
    "--import",
...
console.log(
  `CSI-06 isolated replica proof: ${database}; loopback replica csi01 on port 27189`,
);
child.on("exit", (code) => {
  process.exitCode = code ?? 1;
});
child.on("error", () => {
  console.error("Unable to start isolated CSI-06 replica proof");
  process.exitCode = 1;
});
