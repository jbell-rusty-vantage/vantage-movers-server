...
function putByName(map: Map<string, any[]>, lead: any): void {
  if (
    typeof lead.normalized_name !== "string" ||
    lead.normalized_name.length === 0
  )
    return;
...
export async function reconcileHistoricalRelationships(
  models = registerHistoricalModels(),
) {
