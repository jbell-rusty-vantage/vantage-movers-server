...
export type BackfillWorkbookKey = "top10" | "tbm" | "best_relocation" | "booked";
...
export function getWorkbookConfig(key: BackfillWorkbookKey): BackfillWorkbookConfig {
...
export async function readSpreadsheetTab(
  ...
  const sheetMeta = meta.sheets?.find((sheet) => sheet.properties?.title?.trim() === tabName);
  if (!sheetMeta) {
    throw new Error(`Tab ${tabName} not found in spreadsheet ${spreadsheetId}`);
  }

  const rowCount = sheetMeta.properties?.gridProperties?.rowCount ?? maxScanRows;
...
export async function readWorkbookTab(
  ...
  if (!tab) {
    throw new Error(`Tab ${tabName} is not configured for workbook ${workbookKey}`);
  }
