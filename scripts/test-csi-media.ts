...
// CSI-11 isolated replica proof (directory sync, rebuild, admin routes). Independent of .env and any inherited Atlas URI.
const database = `testvantagemovers_csi11${randomBytes(6).toString("hex")}`;
const child = spawn(
  process.execPath,
  [
    "--import",
  ...
  ],
  {
    ...
    env: {
      ...
      SALES_INTELLIGENCE_DIRECTORY_SYNC: "false", SALES_INTELLIGENCE_MEDIA_ENABLED: "true",
...
console.log(
  `CSI-11 isolated replica proof: ${database}; loopback replica csi01 on port 27189`,
);
child.on("exit", (code) => {
  process.exitCode = code ?? 1;
});
child.on("error", () => {
  console.error("Unable to start isolated CSI-11 replica proof");
  process.exitCode = 1;
});
