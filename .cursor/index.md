...
| [granotLifecycle.desiredState.md](businesslogic/granotLifecycle.desiredState.md) | `src/services/granotLifecycle/leadDesiredState.ts` + `granotTemporal.ts` — origin authority matrix, temporal tuple, pending/unmatched, minimum data; **plans only, no writes** |
