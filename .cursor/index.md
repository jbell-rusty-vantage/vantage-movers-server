...
| [domainCommands.service.md](businesslogic/domainCommands.service.md) | `src/services/domainCommands/` — transaction-owning executor, existing-write adapters (including trusted Ingestion Origin on Form/Call create), append-only `EntityChange`, queued outbox atomicity, post-commit finalize; `synchronizeLeadFromGranot` and `createLeadFromGranot` are registered; owner Booking/Release commands remain disabled |
...
| [granotLifecycle.projections.md](businesslogic/granotLifecycle.projections.md) | `src/services/granotLifecycle/projections.ts` + protected Admin routes — masked case/candidate DTOs and stable Job/Lead timelines; **read-only, no commands/effects** |
