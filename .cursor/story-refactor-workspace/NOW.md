# NOW

- Date (UTC): 2026-09-04
- Wave: A (`src/services/`)
- Current service: `operationsRegistry` (in-progress)
- Next module: `cplSchedule.ts`
- Stock: visited 23 / in-progress 1 / unvisited 14
- Recommendations on disk: 142 (`form-lead.md` through `operations-registry-source-resolution.md`)
- Lock: none
- Lock holder:
- Lock at:
- Branch: `docs/story-refactor`
- PR: https://github.com/jbell-rusty-vantage/vantage-movers-server/pull/144
- Last session: story-operations-registry-source-resolution-2026-09-04T0212Z

Do not add narrative here. Stock detail lives in `TRAVERSAL.md`.
