...
- Wave A tour rows 1–39 are `visited` (including leftover-root skip and `conversations`). Rows 40–42 (`extensionUsers`, `jobNumberTimeline`, `tariff`) stay unvisited. Next run enumerates `extensionUsers`. Disk also has unlisted `dailyOperations` — do not open it while listed rows remain. Do not jump an in-progress checklist.
