...
- Wave A tour rows 1–38 are `visited` (including leftover-root skip). Row 39 `conversations` is in-progress (`reads.ts` recommended). Rows 40–42 (`extensionUsers`, `jobNumberTimeline`, `tariff`) stay unvisited until `conversations` is visited. Do not jump an in-progress checklist.
