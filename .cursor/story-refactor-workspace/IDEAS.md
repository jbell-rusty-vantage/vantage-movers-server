...
- Wave A tour rows 1–40 are `visited` (including leftover-root skip, `conversations`, and `extensionUsers`). Rows 41–42 (`jobNumberTimeline`, `tariff`) stay unvisited. Next run enumerates `jobNumberTimeline`. Disk also has unlisted `dailyOperations` — do not open it while listed rows remain. Do not jump an in-progress checklist.
