...
- Wave A tour rows 1–40 are `visited` (including leftover-root skip, `conversations`, and `extensionUsers`). Row 41 `jobNumberTimeline` is **in-progress** (`assemble.ts` + `projector.ts` + `clocks.ts` recommended). Next is `evidence.ts`. Row 42 `tariff` stays unvisited. Disk also has unlisted `dailyOperations` — do not open it while listed rows remain. Do not jump an in-progress checklist.
