...
`granotLifecycle` is **in-progress**. `observability.ts` is recommended. Next module: **`metrics.ts`**. Stay on `granotLifecycle`. Do not write a whole-folder recommendation.
