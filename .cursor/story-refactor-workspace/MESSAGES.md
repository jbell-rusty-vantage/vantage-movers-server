...
`granotLifecycle` is **in-progress**. `bookingOwnerCommands.ts` is recommended. Next module: **`bookingPriorityPairing.ts`**. Stay on `granotLifecycle`. Do not write a whole-folder recommendation.
