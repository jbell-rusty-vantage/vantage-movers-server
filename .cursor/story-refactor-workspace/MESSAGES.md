...
`granotLifecycle` is **in-progress**. `identity.ts` is recommended. Next module: **`granotTemporal.ts`**. Stay on `granotLifecycle`. Do not write a whole-folder recommendation.
...
2. Do not rewrite `form-lead.md` or any prior recommendation, including `granot-lifecycle-source-policy.md` and `granot-lifecycle-identity.md`.
3. Stay on `granotLifecycle`. Next is `granotTemporal.ts`. Wave B is locked.
