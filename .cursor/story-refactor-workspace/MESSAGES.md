...
`granotLifecycle` is **in-progress**. `bookingConfirmation.ts` is recommended. Next module: **`bookingOwnerCommands.ts`**. Stay on `granotLifecycle`. Do not write a whole-folder recommendation.
