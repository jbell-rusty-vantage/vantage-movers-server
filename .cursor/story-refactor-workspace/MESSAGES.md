...
`granotLifecycle` is **in-progress**. `releaseReconciliation.ts` is recommended. Next module: **`releaseOwnerCommands.ts`**. Stay on `granotLifecycle`. Do not write a whole-folder recommendation.
