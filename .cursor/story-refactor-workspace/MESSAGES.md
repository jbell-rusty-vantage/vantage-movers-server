...
`granotLifecycle` is **in-progress**. `automationApply.ts` is recommended. Next module: **`automationCompatibility.ts`**. Stay on `granotLifecycle`. Do not write a whole-folder recommendation.
...
2. Do not rewrite `form-lead.md` or any prior recommendation, including `granot-lifecycle-extension-apply.md` and `granot-lifecycle-automation-apply.md`.
3. Stay on `granotLifecycle`. Next is `automationCompatibility.ts`. Wave B is locked.
