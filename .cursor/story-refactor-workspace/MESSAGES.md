...
`granotLifecycle` is **in-progress**. `discrepancyOwnerCommands.ts` is recommended. Next module: **`discrepancyProjections.ts`**. Stay on `granotLifecycle`. Do not write a whole-folder recommendation.
