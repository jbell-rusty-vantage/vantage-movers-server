...
`granotLifecycle` is **in-progress**. `bookingPriorityPairing.ts` is recommended. Next module: **`referralBooking.ts`**. Stay on `granotLifecycle`. Do not write a whole-folder recommendation.
