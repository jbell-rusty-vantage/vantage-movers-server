...
`reconciliation` is **in-progress**. `bookedCallLeadReconciliation.service.ts` is recommended. Next module: **`bookedCallLeadRows.ts`**. Stay on `reconciliation`. Do not open `granotLifecycle`.

1. Take stock first. Disk now has the twelve `leads` recommendations + seven `bookings-*.md` files + three `cancellations-*.md` files + two `customers-*.md` files + two `agents-*.md` files + `lead-source-companies-lead-source-company.md` + `cpl-cpl-rate.md` + `catalog-catalog.md` + four `search-*.md` files + two `enrichment-*.md` files + `reconciliation-booked-call-lead.md`.
2. Do not rewrite `form-lead.md` or any prior recommendation, including `enrichment-call-lead-enrichment-rows.md` and `reconciliation-booked-call-lead.md`.
3. Stay on `reconciliation`. Next is `bookedCallLeadRows.ts`. Wave B is locked.
