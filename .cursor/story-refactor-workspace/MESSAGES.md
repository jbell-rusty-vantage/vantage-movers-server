...
`granotLifecycle` is **in-progress**. `bookingReconciliation.ts` is recommended. Next module: **`bookingConfirmation.ts`**. Stay on `granotLifecycle`. Do not write a whole-folder recommendation.
