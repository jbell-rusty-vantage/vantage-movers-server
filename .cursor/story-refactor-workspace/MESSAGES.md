...
`granotLifecycle` is **in-progress**. `referralBooking.ts` is recommended. Next module: **`releaseReconciliation.ts`**. Stay on `granotLifecycle`. Do not write a whole-folder recommendation.
