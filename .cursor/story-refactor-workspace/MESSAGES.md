...
`granotLifecycle` is **in-progress**. `discrepancyProjections.ts` is recommended. Next module: **`observability.ts`**. Stay on `granotLifecycle`. Do not write a whole-folder recommendation.
