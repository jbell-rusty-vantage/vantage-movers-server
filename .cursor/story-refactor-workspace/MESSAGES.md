...
`granotLifecycle` is **in-progress**. `releaseOwnerCommands.ts` is recommended. Next module: **`discrepancies.ts`**. Stay on `granotLifecycle`. Do not write a whole-folder recommendation.
