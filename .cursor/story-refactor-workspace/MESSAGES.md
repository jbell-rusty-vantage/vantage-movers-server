...
`granotLifecycle` is **in-progress**. `trustedLeadCreateValidation.ts` is recommended. Next module: **`synchronizeLeadFromGranot.ts`**. Stay on `granotLifecycle`. Do not write a whole-folder recommendation.
