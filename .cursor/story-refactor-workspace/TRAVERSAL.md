...
| Wave | Tree | When |
| --- | --- | --- |
| **A (visited)** | `src/services/` — every folder, then leftover root barrels | Listed rows 1–42 `visited`. Do not reopen. |
| **B (current)** | Remaining `src/`: `routes/` (visited), then `models/` (in-progress), `validation/`, `config/domain/`, `middleware/`, `auth/` | Stay on `models` until every remaining module is checked. Do not open `validation/` while `models` is in-progress. |
...
- `visited` — every remaining module is `recommended` or `skipped`.
...
### 41. `jobNumberTimeline` — large — **visited**
...
| `recent-official-bookings.ts` | recommended → [recommendations/job-number-timeline-recent-official-bookings.md](recommendations/job-number-timeline-recent-official-bookings.md) |

### 42. `tariff` — small — **visited**

Folder: `src/services/tariff/` — parked after listed Wave A (rows 1–38). Thin folder: two story-worthy modules plus a barrel. Do not treat as one recommendation.

| Module | Verdict |
| --- | --- |
| `append.ts` | recommended → [recommendations/tariff-append.md](recommendations/tariff-append.md) |
| `resolveCarrier.ts` | recommended → [recommendations/tariff-resolve-carrier.md](recommendations/tariff-resolve-carrier.md) |
| `index.ts` | skip — barrel |

## Wave B — remaining `src/`

Listed Wave A rows 1–42 are `visited`. `routes` is `visited`. `models` is `in-progress`. Stay here. This checkout has no `src/services/dailyOperations/` and no `daily-operations-admin.routes.ts`.

### 1. `routes` — large — **visited**

Folder: `src/routes/` — many passes. Do not treat as one recommendation. Enumerated 2026-09-11T1145Z. Runtime modules only (not `*.test.ts`). No empty barrel.

| Module | Verdict |
| --- | --- |
| `v1.routes.ts` | recommended → [recommendations/routes-v1.md](recommendations/routes-v1.md) |
| `extension-auth.routes.ts` | recommended → [recommendations/routes-extension-auth.md](recommendations/routes-extension-auth.md) |
| `google-drive-oauth.routes.ts` | recommended → [recommendations/routes-google-drive-oauth.md](recommendations/routes-google-drive-oauth.md) |
| `ringcentral-registry.routes.ts` | recommended → [recommendations/routes-ringcentral-registry.md](recommendations/routes-ringcentral-registry.md) |
| `granot-lifecycle-admin.routes.ts` | recommended → [recommendations/routes-granot-lifecycle-admin.md](recommendations/routes-granot-lifecycle-admin.md) |
| `job-number-timeline-admin.routes.ts` | recommended → [recommendations/routes-job-number-timeline-admin.md](recommendations/routes-job-number-timeline-admin.md) |
| `conversations-admin.routes.ts` | recommended → [recommendations/routes-conversations-admin.md](recommendations/routes-conversations-admin.md) |
| `extension-users-admin.routes.ts` | recommended → [recommendations/routes-extension-users-admin.md](recommendations/routes-extension-users-admin.md) |
| `extension-granot-apply.routes.ts` | recommended → [recommendations/routes-extension-granot-apply.md](recommendations/routes-extension-granot-apply.md) |
| `tariff-adjustments.routes.ts` | recommended → [recommendations/routes-tariff-adjustments.md](recommendations/routes-tariff-adjustments.md) |
| `granot-automation.routes.ts` | recommended → [recommendations/routes-granot-automation.md](recommendations/routes-granot-automation.md) |
| `ingestion.routes.ts` | recommended → [recommendations/routes-ingestion.md](recommendations/routes-ingestion.md) |
| `reporting.routes.ts` | recommended → [recommendations/routes-reporting.md](recommendations/routes-reporting.md) |
| `granot-webhook.routes.ts` | recommended → [recommendations/routes-granot-webhook.md](recommendations/routes-granot-webhook.md) |
| `ringcentral-webhook.routes.ts` | recommended → [recommendations/routes-ringcentral-webhook.md](recommendations/routes-ringcentral-webhook.md) |
| `ringcentral-webhook-local.routes.ts` | recommended → [recommendations/routes-ringcentral-webhook-local.md](recommendations/routes-ringcentral-webhook-local.md) |
| `twilio-message-status.routes.ts` | recommended → [recommendations/routes-twilio-message-status.md](recommendations/routes-twilio-message-status.md) |
| `twilio-voice.routes.ts` | recommended → [recommendations/routes-twilio-voice.md](recommendations/routes-twilio-voice.md) |
| `ringcentral-cron.routes.ts` | recommended → [recommendations/routes-ringcentral-cron.md](recommendations/routes-ringcentral-cron.md) |
| `booking-reconciliation-cron.routes.ts` | recommended → [recommendations/routes-booking-reconciliation-cron.md](recommendations/routes-booking-reconciliation-cron.md) |
| `sheet-sync-cron.routes.ts` | recommended → [recommendations/routes-sheet-sync-cron.md](recommendations/routes-sheet-sync-cron.md) |
| `lead-messaging-cron.routes.ts` | recommended → [recommendations/routes-lead-messaging-cron.md](recommendations/routes-lead-messaging-cron.md) |
| `cpl-correction-cron.routes.ts` | recommended → [recommendations/routes-cpl-correction-cron.md](recommendations/routes-cpl-correction-cron.md) |
| `notification-cron.routes.ts` | recommended → [recommendations/routes-notification-cron.md](recommendations/routes-notification-cron.md) |
| `best-relocation-ingestion-cron.routes.ts` | recommended → [recommendations/routes-best-relocation-ingestion-cron.md](recommendations/routes-best-relocation-ingestion-cron.md) |
| `reporting-cron.routes.ts` | recommended → [recommendations/routes-reporting-cron.md](recommendations/routes-reporting-cron.md) |
| `granot-automation-cron.routes.ts` | recommended → [recommendations/routes-granot-automation-cron.md](recommendations/routes-granot-automation-cron.md) |
| `granot-lifecycle-cron.routes.ts` | recommended → [recommendations/routes-granot-lifecycle-cron.md](recommendations/routes-granot-lifecycle-cron.md) |

### 2. `models` — large — **in-progress**

Folder: `src/models/` — many passes. Do not treat as one recommendation. Enumerated 2026-09-12T1525Z. Runtime modules only (not `*.test.ts`). No empty root barrel.

| Module | Verdict |
| --- | --- |
| `FormLead.ts` | recommended → [recommendations/models-form-lead.md](recommendations/models-form-lead.md) |
| `CallLead.ts` | recommended → [recommendations/models-call-lead.md](recommendations/models-call-lead.md) |
| `BookedLead.ts` | recommended → [recommendations/models-booked-lead.md](recommendations/models-booked-lead.md) |
| `CancelledLead.ts` | recommended → [recommendations/models-cancelled-lead.md](recommendations/models-cancelled-lead.md) |
| `Customer.ts` | recommended → [recommendations/models-customer.md](recommendations/models-customer.md) |
| `Agent.ts` | recommended → [recommendations/models-agent.md](recommendations/models-agent.md) |
| `LeadSourceCompany.ts` | recommended → [recommendations/models-lead-source-company.md](recommendations/models-lead-source-company.md) |
| `LeadSourceGranularity.ts` | recommended → [recommendations/models-lead-source-granularity.md](recommendations/models-lead-source-granularity.md) |
| `LeadSourceLabelMapping.ts` | recommended → [recommendations/models-lead-source-label-mapping.md](recommendations/models-lead-source-label-mapping.md) |
| `CplRate.ts` | recommended → [recommendations/models-cpl-rate.md](recommendations/models-cpl-rate.md) |
| `CplRatePeriod.ts` | recommended → [recommendations/models-cpl-rate-period.md](recommendations/models-cpl-rate-period.md) |
| `CplCorrectionJob.ts` | recommended → [recommendations/models-cpl-correction-job.md](recommendations/models-cpl-correction-job.md) |
| `CplLeadCorrection.ts` | recommended → [recommendations/models-cpl-lead-correction.md](recommendations/models-cpl-lead-correction.md) |
| `Merchant.ts` | recommended → [recommendations/models-merchant.md](recommendations/models-merchant.md) |
| `MovingCarrier.ts` | recommended → [recommendations/models-moving-carrier.md](recommendations/models-moving-carrier.md) |
| `ExtensionUser.ts` | recommended → [recommendations/models-extension-user.md](recommendations/models-extension-user.md) |
| `Testimonial.ts` | recommended → [recommendations/models-testimonial.md](recommendations/models-testimonial.md) |
| `schemaHelpers.ts` | recommended → [recommendations/models-schema-helpers.md](recommendations/models-schema-helpers.md) |
| `WordpressFormSubmissionReceipt.ts` | recommended → [recommendations/models-wordpress-form-submission-receipt.md](recommendations/models-wordpress-form-submission-receipt.md) |
| `GranotObservationReceipt.ts` | recommended → [recommendations/models-granot-observation-receipt.md](recommendations/models-granot-observation-receipt.md) |
| `GranotObservation.ts` | recommended → [recommendations/models-granot-observation.md](recommendations/models-granot-observation.md) |
| `SynchronizationDecision.ts` | recommended → [recommendations/models-synchronization-decision.md](recommendations/models-synchronization-decision.md) |
| `GranotLifecycleActivation.ts` | recommended → [recommendations/models-granot-lifecycle-activation.md](recommendations/models-granot-lifecycle-activation.md) |
| `GranotRecordLink.ts` | recommended → [recommendations/models-granot-record-link.md](recommendations/models-granot-record-link.md) |
| `GranotBookingReconciliationCase.ts` | recommended → [recommendations/models-granot-booking-reconciliation-case.md](recommendations/models-granot-booking-reconciliation-case.md) |
| `GranotReleaseReconciliationCase.ts` | recommended → [recommendations/models-granot-release-reconciliation-case.md](recommendations/models-granot-release-reconciliation-case.md) |
| `GranotBookingDiscrepancy.ts` | recommended → [recommendations/models-granot-booking-discrepancy.md](recommendations/models-granot-booking-discrepancy.md) |
| `GranotReleaseDiscrepancy.ts` | recommended → [recommendations/models-granot-release-discrepancy.md](recommendations/models-granot-release-discrepancy.md) |
| `granotLifecycleSchemas.ts` | recommended → [recommendations/models-granot-lifecycle-schemas.md](recommendations/models-granot-lifecycle-schemas.md) |
| `granotDiscrepancyModel.ts` | recommended → [recommendations/models-granot-discrepancy-model.md](recommendations/models-granot-discrepancy-model.md) |
| `GranotCrmSource.ts` | recommended → [recommendations/models-granot-crm-source.md](recommendations/models-granot-crm-source.md) |
| `granotCrmSourceSemantics.ts` | recommended → [recommendations/models-granot-crm-source-semantics.md](recommendations/models-granot-crm-source-semantics.md) |
| `GranotCrmCsvIngestion.ts` | recommended → [recommendations/models-granot-crm-csv-ingestion.md](recommendations/models-granot-crm-csv-ingestion.md) |
| `GranotCrmSyncRun.ts` | recommended → [recommendations/models-granot-crm-sync-run.md](recommendations/models-granot-crm-sync-run.md) |
| `GranotAutomationRun.ts` | recommended → [recommendations/models-granot-automation-run.md](recommendations/models-granot-automation-run.md) |
| `GranotAutomationSource.ts` | recommended → [recommendations/models-granot-automation-source.md](recommendations/models-granot-automation-source.md) |
| `SheetSyncJob.ts` | recommended → [recommendations/models-sheet-sync-job.md](recommendations/models-sheet-sync-job.md) |
| `SheetSyncRun.ts` | recommended → [recommendations/models-sheet-sync-run.md](recommendations/models-sheet-sync-run.md) |
| `SheetSyncAttempt.ts` | recommended → [recommendations/models-sheet-sync-attempt.md](recommendations/models-sheet-sync-attempt.md) |
| `SheetSyncLease.ts` | recommended → [recommendations/models-sheet-sync-lease.md](recommendations/models-sheet-sync-lease.md) |
| `SheetSyncQuotaBucket.ts` | recommended → [recommendations/models-sheet-sync-quota-bucket.md](recommendations/models-sheet-sync-quota-bucket.md) |
| `GoogleDriveConnection.ts` | recommended → [recommendations/models-google-drive-connection.md](recommendations/models-google-drive-connection.md) |
| `GoogleOAuthState.ts` | recommended → [recommendations/models-google-oauth-state.md](recommendations/models-google-oauth-state.md) |
| `GooglePickerNonce.ts` | recommended → [recommendations/models-google-picker-nonce.md](recommendations/models-google-picker-nonce.md) |
| `GooglePickerSelection.ts` | recommended → [recommendations/models-google-picker-selection.md](recommendations/models-google-picker-selection.md) |
| `RingCentralInboundRoute.ts` | recommended → [recommendations/models-ringcentral-inbound-route.md](recommendations/models-ringcentral-inbound-route.md) |
| `RingCentralInboundRouteAssignment.ts` | recommended → [recommendations/models-ringcentral-inbound-route-assignment.md](recommendations/models-ringcentral-inbound-route-assignment.md) |
| `LeadMessage.ts` | recommended → [recommendations/models-lead-message.md](recommendations/models-lead-message.md) |
| `LeadMessageRateLimit.ts` | recommended → [recommendations/models-lead-message-rate-limit.md](recommendations/models-lead-message-rate-limit.md) |
| `LeadConversation.ts` | recommended → [recommendations/models-lead-conversation.md](recommendations/models-lead-conversation.md) |
| `BookingLeadReconciliationCase.ts` | recommended → [recommendations/models-booking-lead-reconciliation-case.md](recommendations/models-booking-lead-reconciliation-case.md) |
| `PublicSubmissionThrottleBucket.ts` | recommended → [recommendations/models-public-submission-throttle-bucket.md](recommendations/models-public-submission-throttle-bucket.md) |
| `EntityChange.ts` | recommended → [recommendations/models-entity-change.md](recommendations/models-entity-change.md) |
| `DomainCommandExecution.ts` | recommended → [recommendations/models-domain-command-execution.md](recommendations/models-domain-command-execution.md) |
| `ExternalDataConnection.ts` | recommended → [recommendations/models-external-data-connection.md](recommendations/models-external-data-connection.md) |
| `IngestionRun.ts` | |
| `IngestionConflict.ts` | |
| `SourceRowReceipt.ts` | |
| `SourceRowState.ts` | |
| `ReportingDefinition.ts` | |
| `ReportingDefinitionRevision.ts` | |
| `ReportingPreview.ts` | |
| `ReportingRun.ts` | |
| `ReportingDelivery.ts` | |
| `ReportingDestination.ts` | |
| `ReportingRunConfirmation.ts` | |
| `ReportingRunManifest.ts` | |
| `OperationalEvent.ts` | |
| `OperationalIncident.ts` | |
| `OperationalReportRun.ts` | |
| `NotificationDelivery.ts` | |
| `observabilityModelFactory.ts` | |
| `OperationsRegistryChange.ts` | |
| `historical/index.ts` | skip — barrel |
| `historical/schemaHelpers.ts` | |
| `historical/FormLead.ts` | |
| `historical/CallLead.ts` | |
| `historical/BookedLead.ts` | |
| `historical/CancelledLead.ts` | |
| `historical/Customer.ts` | |
| `historical/Agent.ts` | |

### 3. `validation` — unvisited

Folder: `src/validation/`

### 4. `config/domain` — unvisited

Folder: `src/config/domain/`

### 5. `middleware` — unvisited
