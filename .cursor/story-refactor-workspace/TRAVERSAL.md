...
| **B (next)** | Remaining `src/`: `routes/`, `models/`, `validation/`, `config/domain/`, `middleware/`, `auth/` | Next run opens `src/routes/` first. |
...
## Wave B — next (listed Wave A visited)

Listed Wave A rows 1–42 are `visited`. Next run opens `src/routes/` first. Do not jump ahead to `models/` / `validation/` / `config/domain/` / `middleware/` / `auth/` while routes has unchecked modules.
