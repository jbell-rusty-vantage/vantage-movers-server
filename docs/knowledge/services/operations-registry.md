...
- `ownerGranotNames.ts` — Owner create translation (`OwnerGranotNameCommand`) for `POST /api/v1/admin/granot-crm-sources`. Accepts `when_lead_arrives: create_if_missing`. Best Relocation may use that policy on Forms and Inbounds. Mapped inbound families stay `link_only` unless a later Owner command says otherwise; Call Qualification remains their Call Lead qualifier, and RingCentral-first synchronize / Granot-first adoption stay in place if create is later re-enabled. Derives `crm_origin`, `workspace_slug`, and the Lead Source from the Feed; clients cannot submit `normalized_granot_label`. New rows are inactive (`enabled` / `lifecycle_enabled` false). SMS is not written on create. `validateGranotCrmSourceSemantics` wins if translation disagrees.
...
- Reviewed classification (checked-in manifest): Best Relocation Call/Form and Paid Overflow are `source_scoped_lead` + `create_if_missing`. Main Site Forms, TBM Forms, TBM Forms Prime, Top10 Forms, Main Site Inbounds, 10best Inbounds (TBM Call), TBM Prime Inbounds, and Top10 Inbounds stay `source_scoped_lead` + `link_only` after the 2026-09-03 revert (`pnpm migration:granot-inbound-call-creation-policy-revert`). Referral is `referral_booking` / `observation_only`. Source label Auto stays deferred. `lead_created_policy` stays exactly three values (`link_only` | `create_if_missing` | `observation_only`). There is no fourth value, inbound mint boolean, or ninth gate. `requested_effect` stays `"lead_created"` when creation is eligible. Create stays `lead_created` only. Best Relocation Forms and Inbounds keep `create_if_missing`. Customer text stays a separate `outbound_sms` command. Paid Overflow is created by `pnpm migration:paid-overflow-source` and has no dedicated source sheet.
...
- Classification apply is `scripts/migrations/granot-lifecycle-source-registry.ts` (`pnpm migration:granot-lifecycle:sources -- --report|--apply|--verify`). Report is default. Apply requires `--confirm-production=<db>` and separate authorization. `--scope=best_relocation_creation_policy` and `--scope=link_only_automation_sources` are the only scoped modes. Required Source Company / Source Granularity dependencies resolve per reviewed family company slug, not only Best Relocation. Unique normalized-label index apply is refused while collisions exist. Production `vantagemovers` already applied the `link_only_automation_sources` classification through this audited command. The 2026-09-02 inbound Call `create_if_missing` flip is reversed by `pnpm migration:granot-inbound-call-creation-policy-revert`. That revert does not touch Best Relocation. // pragma: allowlist secret
