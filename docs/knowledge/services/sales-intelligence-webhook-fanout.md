...
- CC-08: webhook projection never settles a row; only a Call Log record does (the refresh or the reconcile). A session gets at most one `call_log_refresh` (dedupe key `...:1`); a later hang-up delivery for the same session reuses it and never conflicts. The refresh applies each record with `source: "call_log_reconcile"` and `proof_ref: "call_log_refresh:<record id>"`, `request_id` = the job id. No record yet: retry at +2, +5, +15 min, then complete with `result.state = "not_published"` (the reconcile window covers it). A 429 (provider, or the shared rate gate refusing the low-priority send) is a `throttled` deferral (attempt not spent) by Retry-After plus up to 5 min jitter, and the recovery drain stops at the first one. A job older than 45 min completes `expired`, and one whose row is already `call_log_state: settled` completes `already_settled`, both without a provider read (the reconcile window covers them). Other provider errors retry with the job backoff; `identity_missing` / `projection_failed` / `account_mismatch` complete with the code on the row.
