...
3. **Estimate** (`prepareManualRun` without `confirmationToken`): `idempotencyKey` required. Binds actor + revision checksum + **stable destination identity** (not volatile `healthVerifiedAt` / `denylistCheckedAt`) + query checksum. Empty `filters: {}` is omitted by Mongo minimize; revision checksum treats missing filters as `{}`. Confirmation TTL 10 minutes. Reusing the key with different immutable inputs → 409 `idempotency_fingerprint_mismatch`.
