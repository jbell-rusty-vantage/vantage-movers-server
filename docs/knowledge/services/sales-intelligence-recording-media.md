...
- 404 when the conversation is missing, `media.blob_pathname` is null, `media.purged_at` or `content_purged_at` is set, or the Number is purged / purge-pending. 416 (`Content-Range: bytes */{size}`) for an unsatisfiable single range; malformed or multi-range headers are ignored (served as if no Range was sent).
- **Bounded responses.** A Vercel function response is capped at 4.5 MB, and browsers open `<audio>` with `bytes=0-`, so every response carries at most `MEDIA_MAX_RESPONSE_BYTES` (2 MiB). A range is clamped to 2 MiB from its start. An un-ranged request for a larger object gets its first chunk as a 206 with the real total, and the player requests the next range. A recording under 2 MiB is served whole with 200. Verified in production on 2026-09-24 with a 16.2 MB recording: 206 chunks of 2,097,152 B, 0.6–1.9 s each.
