...
**Triggers:** `POST /api/v1/form-leads` → `ingestFormLead` (legacy alias `createFormLead`); `PATCH /api/v1/form-leads/:id` → `correctFormLead` (legacy alias `updateFormLead`). Canonical ingest/admin wrappers call `beginFormLeadIngestion` / `correctFormLead` with the executor `{ session, now }` and `completeFormLeadIngestion` only after commit ([`domain-commands.md`](./domain-commands.md)). Public routes still own `runSheetSyncWrite`.
