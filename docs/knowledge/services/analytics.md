...
Production-only. Requires explicit `from`/`to`. Optional `agents[]` filter. Counts booked deals per agent from unwound allocations; **`leads` = `booked_deals`** (no standalone lead attribution). Separate CSV export route. // pragma: allowlist secret
