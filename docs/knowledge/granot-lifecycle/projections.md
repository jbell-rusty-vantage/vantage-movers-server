...
- Owner only: `GET .../cases/:case_id/candidates`, `GET .../cases/:case_id/creating-observation`, `GET .../receipts/live` (SSE; see [live-receipts.md](./live-receipts.md)), `GET .../receipts` (historical webhook-channel list; same doc, not this projector), `GET /api/v1/admin/bookings/:bookingId/connect-lead-candidates` (`listConnectLeadCandidates`; empty `q` is an empty page — see [`bookings.md`](../services/bookings.md)); Owner discrepancy mutations `POST .../discrepancies/:id/{re-evaluate,correct-record-link,no-action}`.
...
- Booking-without-Lead detail deep-links the existing Employee Booking Lead Reconciliation work (`/bookings/reconciliation?case=`). It does not invent another matcher or selector, and it is not a [Granot Booking Reconciliation Case](../../../../CONTEXT.md).
...
Each candidate item carries a normalized `contact` of `name`, `phone_number`, and `email`, plus `job_no`, `normalized_job_no`, and `reference` (`ref_no`). Masked discrepancy cards still use `masked_contact_label`; candidates do not. Form and Call items carry `known_contacts`: the live bag stays `form_submitted` in the DTO (Owner UI labels Call **Called**; Form stays Form submitted); `granot` is present only when `granot_contact_snapshot` exists and uses the stored `differs_from_ingested` plus optional `captured_at`. The DTO never sends `observation_id`. Labels stay live. Form and Call `q` (this browser and Connect) both OR the shared `*_LEAD_CONTACT_*_PATHS` plus `job_no` / `ref_no`. Processor identity still omits Call snapshot phone; automatic suggestion still uses Job else operational phone. This is the "detail/candidate may return normalized contact fields when required for explicit Owner work" allowance in Unit 23, and it is why the endpoint is Owner-only. It is normalized Lead field data, never a raw Lead document or receipt payload. List DTOs, `maskContactLabel`, `maskLifecycleContact`, `assertProjectionSafe`, and `JOB_PROJECTION_FORBIDDEN_KEYS` are unchanged.
