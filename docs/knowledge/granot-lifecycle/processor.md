...
**Domain terms used:** [Synchronization Decision](../../../../CONTEXT.md), [Granot Record Link](../../../../CONTEXT.md), [Granot Observation](../../../../CONTEXT.md), [Granot Observation Receipt](../../../../CONTEXT.md), [System of Record](../../../../CONTEXT.md), [Call Lead](../../../../CONTEXT.md), [Caller Match Key](../../../../CONTEXT.md), [RingCentral Call Adoption](../../../../CONTEXT.md), [Source Granularity](../../../../CONTEXT.md)

# Granot lifecycle processor (`granotLifecycle/processor`)

**Role:** Channel-neutral production orchestrator. One receipt becomes one Observation, one Unit 14 identity result, one desired-state plan, and one Synchronization Decision. Historical shadow may establish or confirm a **job-level** Granot Record Link. Authorized live matched-Lead writes enter `synchronizeLeadFromGranot` only. Authorized live `create_if_missing` with no eligible match enters `createLeadFromGranot` only on `lead_created` (Form or Call). `priority_updated` never mints. This module never patches a Lead or official Booking/Cancellation fact itself. // pragma: allowlist secret
...
- **S6-AGENT latest wins (E3/E6), behind `SALES_INTELLIGENCE_RECEIVER_LATEST_WINS` (default off; off is the rule above, byte-identical).** On: the suggestion also **replaces** an automatic receiver when the Agent differs (`receiverReplaceableByGranot`, [`desired-state.md`](./desired-state.md)): never `manual` or a receiver with no recorded source; `granot_username_match` by the temporal winner (an older observation delivered late already plans `stale`); extension and sheet sources only when the observation's `captured_at` is after `receiver_agent_set_at`; `ringcentral_answered` always. The other guards are unchanged (valid Priority, `user`/`rep` agreement, exactly one active Agent). `synchronizeLeadFromGranot` re-checks the same rule against the Lead read in its transaction (a manual edit that landed meanwhile raises the eligibility race). The write is the normal Lead write: one `EntityChange` with every receiver path before/after, which the Outreach scan turns into `crm_receiver` ([Outreach](../services/sales-intelligence-outreach.md)). Every route class (`lead_created`, `priority_updated`, `booking_status_changed`) normalizes `user`/`rep` and reaches the same planner branch, so the rule already applies to all of them
...
- `create_if_missing` with complete minimum data plans immediately `created` / `lead_created_authorized` when `route_event_class` is `lead_created` (any selected model). Call and Form `priority_updated`, `link_only`, and `booking_status_changed` never plan create. `requested_effect` stays `"lead_created"` when `creation_eligibility` is `eligible` — not a ninth gate and not a fourth `lead_created_policy` value. The processor invokes `createLeadFromGranot` only when execution is `live`, `lead_creation_enabled` is true, and every creation gate is allowed. Shadow and gated-off live persist the suppressed/disabled Decision and create nothing
...
**Live invocation requires all of:** `route_event_class` is `lead_created` (any selected model); execution mode `live`; `GRANOT_LIFECYCLE_LEAD_CREATION_ENABLED=true`; every Unit 15/18 creation gate allowed (global creation flag, post-activation, operational + lifecycle enabled, disposition `source_scoped_lead`, active Source Company and Source Granularity, current reviewed policy `create_if_missing`); the complete identity ladder found no eligible target/ambiguity/conflict/Duplicate-or-Bad restriction; `evaluateMinimumCreationData` is `eligible`; the selected Form/Call route is deterministic and agrees with the command model. `requested_effect` stays `"lead_created"` when `creation_eligibility` is `eligible`. `evaluateEffectGates` / `policyPermitsEffect` do not read `route_event_class`; there is no ninth gate. Form and Call `lead_created` + `create_if_missing` stay eligible. `priority_updated`, `link_only`, and `booking_status_changed` never mint. Incomplete immutable data is terminal for this Observation (`insufficient_creation_data` with the exact missing-data reason) and creates no Lead, link, Command, Change, or outbox. `link_only` keeps the Unit 08 pending/unmatched clock. A later complete Observation may still create.

When creating a Call Lead and the Observation has a normalized phone, both Granot lock sites always run: `ensureRingCentralConvergenceScopeLock` before the transaction and `acquireRingCentralConvergenceScopeLock` + `findPreCreationRingCentralConvergenceCandidates` inside it. `RINGCENTRAL_GRANOT_ADOPTION_ENABLED` does not gate those Granot sites. Skip both when there is no normalized phone (Job-only create remains legal — a residual hole; do not invent a phone). Fence hit → `CreateLeadFromGranotRaceError("identity")` → `prepareDecision` replan → `synchronizeLeadFromGranot` when exactly one eligible Lead. Owner language uses [Caller Match Key](../../../../CONTEXT.md); the locked implementation key is exact Source Granularity + normalized phone — never Source Company alone. [RingCentral Call Adoption](../../../../CONTEXT.md) mutations and the ingest-side lock stay flagged; do not describe the ingest lock as always on.
