...
4. **Reverify §4 against the repository before writing code.** It was
   observed on 2026-09-02. Anything that has drifted, you correct in
   the issue in the same change as your work.

Work on the branch this desk already uses for Lead Costs / Operations
Registry work if that branch exists and the tree is otherwise this
work. Create `lead-costs-owner-editing` only when that branch does
not exist and the working tree is clean of unrelated work. Do not
open extra feature branches per issue. Server and Admin are separate
git roots — stay in the issue’s repository.
...
- LCE-01 stays in `vantage-main-server`. LCE-02–04 stay in
  `vantage-admin`. Pack docs may be updated when an issue corrects
  drift.
- Owner-visible strings use spec §8. Do not print `granularity`,
  `correct_period`, `replace_schedule`, or Period IDs on the default
  path.
- 21st.dev (MCP `user-21st` or CLI) is allowed only for the craft
  target named in the issue. Search first. Do not replace
  `CplManager` with a generated page that invents endpoints.
...
1. Write `reports/LCE-0<n>-completion.md` covering the issue’s §14
   handoff list.
...
4. After runtime or Admin UI changes, invoke the **docs-keeper**
   agent so the knowledge pointer / admin map describe the code that
...
- **Never rewrite Lead CPL inside a schedule command.**
- **Never remove the four existing Advanced operations from the API.**
- **Never chain `split` + `correct_period` in the browser to fake a
  date range.** That is why LCE-01 exists.
- **Never enable a write, deploy to production, or read a live
  customer payload** unless the user explicitly asks.
...
Use glossary terms from workspace-root `CONTEXT.md`. Say **CPL** in
domain/server prose. Say **Lead cost** on Owner surfaces. Say
**Feed** not Granularity. Say **CPL Correction** / **Existing leads**
for the backfill, never `correct_period`.
