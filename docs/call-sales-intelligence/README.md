...
| 16 | [`16-mcp-tool-surface-precomputed-context-and-attachment.md`](16-mcp-tool-surface-precomputed-context-and-attachment.md) | Analysis and recommendation: the intelligence MCP tool surface (no MongoDB tools; 91.6% of tool bytes are one duplicated submit schema), per-run tool scoping that already exists but is hardcoded open, precomputing the resolved Lead so the model stops re-discovering it, and a review of the Lead matching / attachment logic. |
