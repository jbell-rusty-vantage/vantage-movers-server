...
HTTP 400 `INVALID_INPUT`/`EVIDENCE_SCOPE_INVALID`; 403 scope/authority denial; 409 submission/revision/lease conflict; 413 `EVIDENCE_LIMIT_REACHED`/`BUDGET_EXHAUSTED`; 422 `ORIGINAL_EVIDENCE_UNAVAILABLE`; 503 `PROVIDER_READ_UNAVAILABLE`. Upstream unavailable/disabled provider reads can instead return an incomplete page with a safe missing-range reason. No raw provider body, submitted evidence or credential appears in route errors. Core `ENABLED` and additive `PROVIDER_READS` remain off unless explicitly configured; this task enabled flags only in child test processes.
