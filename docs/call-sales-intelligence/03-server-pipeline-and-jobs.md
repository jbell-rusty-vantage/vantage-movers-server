...
#### CSI-11 implementation policy

CSI-11 implementation policy (September 17): `recording_discovery` and new `media_fetch` jobs run under `MEDIA_ENABLED`; the legacy reserved `media` enum is preserved for compatibility but receives no CSI-11 jobs. Availability window is **72 hours** from the interaction's first observation for absent IDs, and the conversation's creation for recording-content 404s. This is a bounded engineering wait, not a verified RingCentral retention guarantee. Pending waits grow from 1 minute to at most 1 hour. Permission denial waits 24 hours; throttling honors Retry-After or 10 minutes; these and missing eligibility inputs do not burn transient-failure attempts. Unknown eligibility is reconsidered every 10 minutes. All other failures use the shared eight-attempt policy; expired eighth claims receive transactional failure projection in recovery.

The digest cannot be known until the final byte. `services/conversations/streamingMedia.ts` streams to a random mode-0600 bounded temporary file while hashing, streams that file into private Blob with `allowOverwrite:false`, then unlinks it in `finally`. No full audio buffer, provider URL persistence, seed-helper overwrite, STT, or analysis enqueue. Verified content type and file signature select the extension; declared and actual byte limits are enforced. One recording per media invocation, with a 90-second provider/Blob abort signal inside a five-minute job lease. A crash may leave an unreferenced immutable Blob; deployment retention must reconcile these. CSI-12 consumes `media_stored`, digest and `pending_stage: transcription`; CSI-11 only leaves the hook, and suppresses it when current eligibility changed during download.
