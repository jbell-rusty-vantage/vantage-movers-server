...
CSI-05 server implementation evidence (September 18): [handoff](workspace/evidence/csi-05/HANDOFF.md), [checks](workspace/evidence/csi-05/CHECKS.md). Attachment attribution uses event-time windows and pinned exact identities rather than lifetime edge counts; the CSI-11 eligibility reader consumes that resolver. Queue handler, five-minute cron and minute recovery are registered. All attachment commands retain the existing Owner/scope/TEST_MODE boundary and write CSI audit only. EntityChange remains a future applied_at scan source, with no CSI origin writes. Lead watermark scans use independent timestamp/id checkpoints; no Outreach/official-state scanning or writes are implemented by CSI-05. Other audit rows remain owned by their listed teams.
