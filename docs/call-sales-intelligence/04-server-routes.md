...
Outreach DTO includes state/reason, subject, revision, overall assignment, all active follow-ups (paginated detail when large), next-action projection, human-contact timestamps, DerivedDto and related record links. It additionally carries two optional nullable objects, so an already-shipped consumer keeps parsing: `lead_attachment` (`attachment_id`, `lead_ref`, `state`, `certainty`, `certainty_label` from the shared certainty labeller, `decided_by: owner|automatic|evidence`, `decided_at`, `confidence`, `observed_at`, `lead_display`) and `call_progress` (`state`, `started_at`, `started_by`, `ended_at`, `ended_by`, `note`). DerivedDto gains optional `call_state: not_started|in_progress|ended` and `provenance_state: attached_by_you|attached_automatically|attached_from_evidence|needs_a_lead|ambiguous`. `provenance_state` is derived from the stored mirror on read, never stored: Confirmed by you wins, then an automatic decision, then an exact or evidence-derived attach; Ambiguous or competing identity is Ambiguous and anything not Attached is Needs a lead. Outreach detail, by-Lead detail, Number detail and Attention all carry the same fields, and `allowed_actions` includes `start_call`/`end_call` with the same blockers the commands enforce. Both a closed record and a call already in progress use `ILLEGAL_TRANSITION`; `derived.call_state` is what distinguishes those two reasons. Number detail uses `outreach_records[]`, not one misleading singular row when multiple Leads share a number. Lead chips expose attachment certainty (Exact/Likely/Unsure/Confirmed by you), source, official flags and an existing Admin href. Do not label phone equality as confirmed identity.
...
| start_call | optional note | Owner marks that he is on this call now. Writes `call_progress` only; it never changes Outreach state, contacts nobody and completes no action. Blocked when the record is closed (`ILLEGAL_TRANSITION`), when a call is already in progress (`ILLEGAL_TRANSITION`), and by the same active call restriction that gates dialling (`CONTACT_RESTRICTED`). Starting again after an ended call replaces the field; the audit rows are the history. |
| end_call | optional note | Ends the Owner's call progress. Blocked when no call is in progress and when the record is closed (`ILLEGAL_TRANSITION`). A later Call Interaction covering the started window ends it on its own and stamps `interaction_id`, so this is a correction, not a requirement. |
...
| reanalyze | mode original_evidence/current_context, source_run_id when original, Owner correction refs, optional focus_finding_id | Async 202 with run id. Missing purged source → 422 ORIGINAL_EVIDENCE_UNAVAILABLE. `focus_finding_id` must be a live Finding of that run (else RUN_SCOPE_DENIED); it is provenance for why the rerun was asked for and changes neither run scoping nor evidence rules. It is recorded on the request and surfaced on `reanalysis_requests[]`. |
...
### CSI-18 implemented route reconciliation

The Owner analysis routes and exact additive request/response contracts are now registered in `src/routes/sales-intelligence-admin.routes.ts`; see `workspace/CONTRACTS.md` section “CSI-18 executable additive contracts”. Finding confirmation/correction/retraction require `expected_output_digest`. Targeted correction additionally accepts explicit `action_changes` and exact follow-up revision fences. `GET /analysis-runs/:id` includes paged immutable review history and pending refresh requests. Original evidence is manifest-bound and purge-aware. This is implementation documentation; final server/UI acceptance remains recorded separately in `workspace/evidence/csi-18`.
