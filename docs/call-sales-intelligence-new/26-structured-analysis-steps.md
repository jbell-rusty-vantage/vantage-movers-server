...
Pure tests cover compact contracts, citation expansion, artifact integrity, cost measurement, metadata removal, admission and resumable bounded backfill. `scripts/dev_ops/test-csi-structured.ts` runs fourteen real-worker checks against a disposable loopback Mongo replica on port 27189 (replica set `csi01`), with synthetic data and a mocked model that rejects tool usage. It covers normal application, shadow fencing, local repair, checkpoint reuse, purge refusal, number synthesis, original-evidence pinning, provider 429, per-recording reporting and exact monthly reconciliation. Regression checks prove that canonical summaries are erased across all purged transcript versions, original number replay retains its captured prior summary, and a replaced transcript starts a clean number generation instead of mixing checkpoint evidence. Provider retries use separate reservation step keys per lease epoch, preserve uncertain usage from interrupted attempts, reuse saved summaries and still submit once.

Validation on September 22: typecheck and full lint passed; the full offline suite passed 2,528 tests with 114 skips. The required `finish-work --provider codex` checkpoint completed review and checks in an isolated snapshot. Its unconstrained test run hit a timeout in the unrelated request-telemetry child process; that test passed when rerun separately. Its proposed repair-count and hard-spend caps were not applied because they contradict the accepted elapsed-only repair and monthly soft-stop decisions above. The isolated checkpoint patch was not applied to this branch.
