function createCancelledLeadForm() {
  ...
  form
    ...
    .setHelpText(
      "Optional. Person submitting or responsible for the cancellation.",
    )
    .setRequired(false);
  ...
  ScriptApp.newTrigger("onCancelledLeadSubmit")
    .forForm(form)
    .onFormSubmit()
    .create();
...
function onCancelledLeadSubmit(e) {
  ...
  if (status < 200 || status >= 300) {
    throw new Error(
      "Cancelled lead API failed with status " + status + ": " + body,
    );
  }
}
...
function parseCancelledLeadFormValues(e) {
  ...
  if (e && e.namedValues) {
    Object.keys(e.namedValues).forEach((key) => {
      values[key] = Array.isArray(e.namedValues[key])
        ? e.namedValues[key][0]
        : e.namedValues[key];
    });
    return values;
