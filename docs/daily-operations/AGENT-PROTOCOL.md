...
4. **Reverify §4 against the repository before writing code.** It was
   observed on 2026-09-06. Anything that has drifted, you correct in
   the issue in the same change as your work.

Work on the branch this desk already uses for Daily Operations if that
branch exists and the tree is otherwise this work. Create
`daily-operations` only when that branch does not exist and the working
tree is clean of unrelated work. Do not open extra feature branches
per issue. Server and Admin are separate git roots — stay in the
issue’s repository.
...
1. Write `reports/DOP-0<n>-completion.md` covering the issue’s §14
   handoff list.
...
- **Never increment Redis as the Owner-visible total.**
- **Never hook Granot HTTP 202 for Lead / Booking / Decision counts.**
- **Never call the writer from `applyBestRelocationPlan`.**
- **Never implement the 2026-08-19 tabbed 24h/48h Daily View.**
